...
## 프론트엔드 코드 전용 AI 컨텍스트 훅 엔진 (fe-lens)
...
**fe-lens는 Claude Code / Codex / OMC / OMX 같은 AI 코딩 런타임에서 프론트엔드 파일을 읽기 전에, 원문 전체 대신 구조·계약·행동 중심의 압축 컨텍스트를 제공해 토큰 비용을 줄이는 로컬 엔진이다.**
...
런타임은 해당 파일을 fe-lens에 먼저 질의하고, fe-lens는 원문 대신 압축 컨텍스트를 제공한다.
...
fe-lens는 UI를 “그림처럼 자세히 재현”하지 않는다.  
...
```bash
fe-lens extract src/components/Button.tsx --json
```
...
```bash
fe-lens decide src/components/Button.tsx
```
...
특정 런타임이 fe-lens core engine을 호출할 수 있도록 연결부를 설치한다.
