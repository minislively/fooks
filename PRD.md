...
## 프론트엔드 코드 전용 AI 컨텍스트 훅 엔진 (fooks)
...
> 초기 PHASE 1 드래프트에서는 `fe-lens`라는 작업명을 사용했다. 현재 제품/레포/패키지/기본 CLI 이름은 `fooks`이며, `fxxks`와 `fe-lens`는 호환성 alias로만 유지한다.
...
**fooks는 Claude Code / Codex / OMC / OMX 같은 AI 코딩 런타임에서 프론트엔드 파일을 읽기 전에, 원문 전체 대신 구조·계약·행동 중심의 압축 컨텍스트를 제공해 토큰 비용을 줄이는 로컬 엔진이다.**
...
런타임은 해당 파일을 fooks에 먼저 질의하고, fooks는 원문 대신 압축 컨텍스트를 제공한다.
...
```bash
fooks extract src/components/Button.tsx --json
```
...
```bash
fooks decide src/components/Button.tsx
```
...
특정 런타임이 fooks core engine을 호출할 수 있도록 연결부를 설치한다.
