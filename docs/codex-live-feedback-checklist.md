...
이 문서는 `fxxks install codex-hooks` + `fxxks attach codex` 이후,
...
- `~/.codex/hooks.json` 에 `fxxks codex-runtime-hook --native-hook` 가 들어가는가
...
- `#fxxks-full-read` 가 기대대로 full-read 우회를 못 함
